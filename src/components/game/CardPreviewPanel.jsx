...
export default function CardPreviewPanel({ card, playable, blocked, onPlay, onCancel }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 pb-[calc(0.5rem+env(safe-area-inset-bottom))] animate-fade-in" style={{ background: "rgba(8,12,24,0.95)", borderTop: "1px solid rgba(201,168,76,0.2)" }}>
      <div className="max-w-md mx-auto px-3 py-2">
        <div className="flex items-center gap-2.5">
          {/* Compact card thumbnail */}
          <div className={cn("flex-shrink-0 rounded-md border-2 overflow-hidden", rarityBorder)}>
            <div className="relative w-11 h-11" style={{ background: "linear-gradient(160deg, #1a2744, #0f1a30)" }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <h4 className="font-serif text-amber-100 text-xs leading-tight truncate">{card.name}</h4>
              <span className={cn("text-[9px] font-semibold uppercase flex-shrink-0", typeInfo.color)}>{typeInfo.text}</span>
            </div>
            <p className="text-amber-100/70 text-[10px] leading-tight truncate">{effectText}</p>
            <p className="text-amber-300/40 text-[8px] italic truncate">{card.verse}</p>
          </div>
          ...
          <button
            onClick={onPlay}
            disabled={!canPlay}
            className="flex-shrink-0 px-3 py-1.5 rounded-lg border-2 border-amber-400/60 bg-amber-600/20 text-amber-100 font-bold text-xs hover:bg-amber-600/40 transition disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
          >
            Play
          </button>
          <button
            onClick={onCancel}
            className="flex-shrink-0 px-3 py-1.5 rounded-lg border border-slate-500/40 bg-slate-800/40 text-amber-100/70 text-xs hover:bg-slate-700/40 transition"
          >
            Cancel
          </button>
        </div>
