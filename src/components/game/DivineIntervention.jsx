...
const BLESSING_STYLES = {
  ...
  wisdom: {
    ...
    artBorder: "border-emerald-400/40",
  },
  covenant: {
    category: "Wisdom / Draw",
    icon: BookOpen,
  ...
  manna: {
    ...
    artBorder: "border-amber-400/40",
  },
};

function getBlessingStyle(blessingId) {
  return BLESSING_STYLES[blessingId] || BLESSING_STYLES.manna;
}
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden" style={{ background: "radial-gradient(ellipse at center, rgba(201,168,76,0.15) 0%, rgba(8,12,24,0.98) 60%)" }}>
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl w-full">
        {blessings.map((blessing) => {
          const style = getBlessingStyle(blessing.id);
          const CategoryIcon = style.icon;
          const isChosen = chosen === blessing.id;
          ...
          return (
            <button
              ...
              className={`p-6 rounded-xl border-2 text-center transition-all duration-300 ${
                isChosen
                  ? `${style.border} ${style.accentBg} scale-105 ${style.glow}`
                  : isDimmed
                  ? "border-slate-700/30 opacity-30"
                  : `${style.border} bg-amber-900/10 hover:scale-105 cursor-pointer ${style.glow}`
              }`}
              style={{ background: isChosen || isDimmed ? undefined : "linear-gradient(135deg, rgba(26,39,68,0.8) 0%, rgba(15,26,48,0.8) 100%)" }}
            >
              {/* Category label */}
              <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-3 ${style.accentBg} ${style.accentText} ${style.border} border`}>
